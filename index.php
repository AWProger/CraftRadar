...
$topVotes = cacheRemember('home_top_votes', 300, function() use ($db) {
    return $db->query("
        SELECT id, name, ip, port, icon, is_online, players_online, players_max, votes_total, rating, is_verified
        FROM servers WHERE status IN ('active', 'pending') 
        ORDER BY is_promoted DESC, votes_total DESC 
        LIMIT 10
    ")->fetchAll();
});
...
$topOnline = cacheRemember('home_top_online', 120, function() use ($db) {
    return $db->query("
        SELECT id, name, ip, port, icon, is_online, players_online, players_max, votes_total, is_verified
...
$newServers = cacheRemember('home_new_servers', 300, function() use ($db) {
    return $db->query("
    SELECT id, name, ip, port, icon, is_online, players_online, players_max, votes_total, is_verified, created_at
    FROM servers WHERE status IN ('active', 'pending') 
