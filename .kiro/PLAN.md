...
- [x] 6.4 Файловый кэш для главной и каталога
- [x] 6.5 SEO (title, description, OpenGraph)
- [x] 6.6 `page.php` — статические страницы
- [x] 6.7 Страница 404

## Этап 7 — ЮMoney платежи (продвижение серверов)
- [x] 7.1 Таблица `payments` в БД + обновление database.sql
- [x] 7.2 `includes/yoomoney.php` — класс интеграции с ЮMoney (создание платежа, проверка уведомления)
- [x] 7.3 Настройки ЮMoney в config.php (wallet, secret, notification URL)
- [x] 7.4 `dashboard/promote.php` — страница покупки продвижения (выбор тарифа, оплата)
- [x] 7.5 `payment/notify.php` — обработчик уведомлений от ЮMoney (webhook)
- [x] 7.6 `payment/success.php` + `payment/fail.php` — страницы после оплаты
- [x] 7.7 `admin/payments.php` — управление платежами в админке
- [x] 7.8 Графики доходов в админ-дашборде
- [x] 7.9 Тесты для платёжной системы

## Этап 8 — Улучшение админки и статистики
- [x] 8.1 Графики в админ-дашборде (Chart.js): регистрации, серверы, голоса по дням
- [x] 8.2 Графики доходов по дням/месяцам
- [x] 8.3 Экспорт лога в CSV
- [x] 8.4 Тесты для улучшений

## Этап 9 — Улучшения UX
- [x] 9.1 Система уведомлений в кабинете (сервер оффлайн, отклонён, новый отзыв)
- [x] 9.2 Живой AJAX-поиск с автодополнением
- [x] 9.3 Баннеры/виджеты для вставки на сайт владельца
- [x] 9.4 Переключение графиков 24ч/7д/30д на странице сервера
- [x] 9.5 Тесты

## Этап 10 — Minecraft-стиль + система баллов
- [x] 10.1 Полный редизайн CSS — блочный Minecraft-стиль (пиксельные элементы, текстуры, шрифт Minecraft)
- [x] 10.2 Онлайн-баннеры серверов (динамические, с реальным онлайном)
- [x] 10.3 Система баллов — начисление за голосование, трата на выделение сервера
- [x] 10.4 Таблица `user_points` + `point_transactions` в БД
- [x] 10.5 Страница «Выделить сервер за баллы» в кабинете
- [x] 10.6 Тесты
